...
QUESTION 2:
Write a program to perform CRUD operations on Employee table using
...
<form id="form1" runat="server">
<div>
<asp:SqlDataSource ID="EmpSDS" runat="server"
ConnectionString="Data Source=DESKTOP-IKKO8FF\SQLEXPRESS;Initial
Catalog=assignment;Integrated Security=True;"
SelectCommand="Select * from Emp"
InsertCommand="insert into Emp values(@empid, @name, @contact, @dept, @sal)"
UpdateCommand="update Emp set name=@name, contact=@contact, dept=@dept
,sal=@sal where empid=@empid"
...
<asp:GridView ID="EmpGV" runat="server"
AutoGenerateColumns="true"
DataKeyNames="empId"
DataSourceId="empSDS"
AutoGenerateEditButton="true"
AutoGenerateDeleteButton="true"
AutoGenerateInsertButton="true"></asp:GridView>
</div>
</form>
</body>
</html>
...
QUESTION 3:
Write a program to display a list of departments in DropDownList. When
the user selects the department, display the list of all employees working in the
selected department. Use GridView to show the records.
ASPX:
<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="WebForm1.aspx.cs"
Inherits="WebApplication2.WebForm1" %>
<!DOCTYPE html>
...
OnSelectedIndexChanged="func" DataSourceID="DeptSDS" DataTextField="dept"
DataValueField="dept"></asp:DropDownList>
<asp:SqlDataSource ID="DeptSDS" runat="server"
ConnectionString="Data Source=DESKTOP-IKKO8FF\SQLEXPRESS;Initial
Catalog=assignment;Integrated Security=True;"
SelectCommand="Select distinct dept from Emp">
</asp:SqlDataSource>
<asp:SqlDataSource ID="EmpSDS" runat="server"
ConnectionString="Data Source=DESKTOP-IKKO8FF\SQLEXPRESS;Initial
Catalog=assignment;Integrated Security=True;"
SelectCommand="Select * from Emp where dept=@dept">
<SelectParameters>
<asp:ControlParameter ControlID="DDL" Name="dept"
...
<asp:GridView ID="EmpGV" runat="server"
AutoGenerateColumns="true"
DataSourceId="EmpSDS"></asp:GridView>
</div>
</form>
...
QUESTION 4:
Write a program to perform CRUD operations on Employee table without using
SqlDataSource and Gridview.
ASPX:
<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="WebForm1.aspx.cs"
Inherits="WebApplication2.WebForm1" %>
...
<asp:Label ID="lblDepartment" runat="server" Text="Select Department:" />
<asp:DropDownList ID="DDL" runat="server" AutoPostBack="true"
OnSelectedIndexChanged="DDL_SelectedIndexChanged"></asp:DropDownList>
<hr />
<!-- GridView to Display Employees -->
<asp:GridView ID="EmpGV" runat="server" AutoGenerateColumns="False"
OnRowCommand="EmpGV_RowCommand">
<Columns>
...
public partial class WebForm1 : System.Web.UI.Page
{
private string connectionString = "Data Source=DESKTOP-IKKO8FF\\SQLEXPRESS;Initial
Catalog=assignment;Integrated Security=True;";
protected void Page_Load(object sender, EventArgs e)
{
if (!IsPostBack)
...
SqlCommand cmd = new SqlCommand("SELECT DISTINCT dept FROM Emp",
conn.Open();
DDL.DataSource = cmd.ExecuteReader();
DDL.DataTextField = "dept";
DDL.DataValueField = "dept";
DDL.DataBind();
DDL.Items.Insert(0, new ListItem("Select Department", ""));

private void LoadEmployees(string department = null)
{
using (SqlConnection conn = new SqlConnection(connectionString))
{
...
{
LoadEmployees(DDL.SelectedValue);
}
...
using (SqlConnection conn = new SqlConnection(connectionString))
{
SqlCommand cmd = new SqlCommand("INSERT INTO Emp (empid, name, contact,
dept, sal) VALUES (@empid, @name, @contact, @dept, @sal)", conn);
...
using (SqlConnection conn = new SqlConnection(connectionString))
{
SqlCommand cmd = new SqlCommand("UPDATE Emp SET name=@name,
contact=@contact, dept=@dept, sal=@sal WHERE empid=@empid", conn);
cmd.Parameters.AddWithValue("@empid", txtEmpID.Text);
cmd.Parameters.AddWithValue("@name", txtName.Text);
cmd.Parameters.AddWithValue("@contact", txtContact.Text);
cmd.Parameters.AddWithValue("@dept", txtDept.Text);
...
protected void EmpGV_RowCommand(object sender,
System.Web.UI.WebControls.GridViewCommandEventArgs e)
{
int rowIndex = Convert.ToInt32(e.CommandArgument);
int empid = Convert.ToInt32(EmpGV.DataKeys[rowIndex].Value);
...
using (SqlConnection conn = new SqlConnection(connectionString))
{
SqlCommand cmd = new SqlCommand("SELECT * FROM Emp WHERE empid =
@empid", conn);
...
txtSalary.Text = reader["sal"].ToString();
}
}
}
